...
def test_process_claimed_job_prints_once_and_returns_result(monkeypatch):
    ...
    monkeypatch.setattr(
        polling_client,
        "print_single_job",
        lambda job, cfg: calls.append(job["name"]) or job["printer_name"],
    )
    job = {
        "name": "BCN-PRINT-JOB-00001",
...
def test_flush_pending_result_timeout_does_not_reprint(monkeypatch):
    ...
    pending = polling_client.PendingResult(
        "BCN-PRINT-JOB-00001", "Printed", ""
    )
...
def test_loop_null_job_sleeps_default_interval(monkeypatch):
    ...
    monkeypatch.setattr(
        polling_client,
        "claim_job",
        lambda session, cfg, printers: calls.__setitem__(
            "claim", calls["claim"] + 1
        ),
    )
    monkeypatch.setattr(
    ...
    polling_client.run_polling_loop(
        cfg,
        lambda: ["Kitchen Printer"],
        lambda: next(stop_checks),
    )
