...
def test_build_ticket_html_uses_compact_waiter_layout():
    ...
    assert '<table class="ticket-head">' not in html
    assert 'text-align: right' not in html
    assert '.ticket-head { margin: 1px 0; white-space: nowrap;' in html
    assert '.table-name { font-size: 13px; font-weight: 700;' in html
    assert '.order-datetime { font-size: 12px; margin-left: 10px; white-space: nowrap;' in html
